...
"use strict";
const verify = (result)=>{return result.trimEnd().match(/--- Verification by the evaluation system\s+7\s*$/);}
const fs = require('fs');
...
cat << "EOF" > ~/"${JUDGE_DIR}/verify.js"
"use strict";
const verify = (result)=>{return result.trimEnd().match(/--- Verification by the evaluation system\s+11\s*$/);}
const fs = require('fs');
const result = verify(fs.readFileSync("output.txt", "utf8"));
if (result) {
    process.stdout.write("1\n");
...
cat << "EOF" > ~/"${JUDGE_DIR}/verify.js"
"use strict";
const verify = (result)=>{return result.trimEnd().match(/--- Verification by the evaluation system\s+13\s*$/);}
const fs = require('fs');
const result = verify(fs.readFileSync("output.txt", "utf8"));
if (result) {
    process.stdout.write("1\n");
...
cat << "EOF" > ~/"${JUDGE_DIR}/verify.js"
"use strict";
const verify = (result)=>{return result.trimEnd().match(/--- Verification by the evaluation system\s+19\s*$/);};
const fs = require('fs');
const result = verify(fs.readFileSync("output.txt", "utf8"));
if (result) {
    process.stdout.write("1\n");
